#!/usr/bin/bash
  ...
  pip install poetry-git-version-plugin
  ...
  while [ "$(curl -s -o /dev/null -w "%{http_code}" $url/login?from=%2F)" != "200" ]; do sleep 1; done
  ...
  java -jar jenkins-cli.jar -s http://localhost:8080/ groovy = < setup.groovy
