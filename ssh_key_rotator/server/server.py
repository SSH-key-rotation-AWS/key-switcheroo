...
class Server:
    ...
    def __setup_authorized_keys_script(self):
        ...
        subprocess.run(f"sudo cp {python_retrieval_script_path} {target_path}",
                        shell=True,
                        check=True)
        subprocess.run(f"sudo chgrp 0 {python_retrieval_script_path}", check=True, shell=True)
        subprocess.run(f"sudo chmod 755 {python_retrieval_script_path}", check=True, shell=True)
